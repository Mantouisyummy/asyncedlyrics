...
class NetEase(LRCProvider):
    ...
    async def search_track(self, search_term: str) -> Optional[dict]:
        ...
        async with self.session.get(self.API_ENDPOINT_METADATA, params=params) as response:
            try:
    ...
    async def get_lrc_by_id(self, track_id: str) -> Optional[Lyrics]:
        ...
        async with self.session.get(self.API_ENDPOINT_LYRICS, params=params) as response:
            try:
