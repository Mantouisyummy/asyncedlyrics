...
class Musixmatch(LRCProvider):
    ...
    async def _get_token(self):
        ...
        d = await self._get("token.get", [("user_language", "en")])
    ...
    async def get_lrc_by_id(self, track_id: str) -> Optional[Lyrics]:
        ...
        if self.lang is not None:
            ...
            body_tr = r_tr.get("message", {}).get("body", {})
            if not body_tr.get("translations_list"):
                raise Exception("Couldn't find translations")
        if not r:
            return None
        body = r.get("message", {}).get("body", {})
        if not body:
            return None
    ...
    async def get_lrc_word_by_word(self, track_id: str) -> Optional[Lyrics]:
        lrc = Lyrics()
        r = await self._get("track.richsync.get", [("track_id", track_id)])
        if r and r.get("message", {}).get("header", {}).get("status_code") == 200:
            lrc_raw = r.get("message", {}).get("body", {}).get("richsync", {}).get("richsync_body", "")
    ...
    async def get_lrc(self, search_term: str) -> Coroutine[Any, Any, Optional[Lyrics]] | None:
        ...
        status_code = r.get("message", {}).get("header", {}).get("status_code")
        if status_code != 200:
        ...
        if self.enhanced:
            lrc = await self.get_lrc_word_by_word(track_id)
            if lrc and lrc.synced:
                return lrc
        return await self.get_lrc_by_id(track_id)